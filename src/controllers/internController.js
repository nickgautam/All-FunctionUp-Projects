...
const createIntern = async function (req, res) {
  ...
  if ((internData.email).includes(" ")){{return res.status(400).send({ status: false, message: "Please remove any empty spaces in email" });}}
   ...
   if (!internData.mobile)
    ...
    if (typeof (internData.mobile) != "number"){ return res.status(400).send({ status: false, message: "mobile must be a number" });}
    if ((internData.mobile).trim().length === 0){{return res.status(400).send({ status: false, message: "mobile can't be empty" });}}
    if (isNum(internData.mobile) === false) { return res.status(400).send({ status: false, message: "mobile must be a number" });}
    if ((internData.mobile).includes(" ")){{return res.status(400).send({ status: false, message: "Please remove any empty spaces in name" });}}
